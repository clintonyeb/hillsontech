...
exports.payment = function(request, reply) {
  ...
  request.server.app.db.users.findOne(
    ...
    (err, doc) => {
      if (err) return errorReply(reply, "apply-pay", doc);
      return successReply(reply, "apply-pay", doc);
    }
  );
};

exports.getPDF = generatePDF;

exports.pdfFile = function(request, reply) {
  ...
  request.server.app.db.users.findOne(
    ...
    (err, doc) => {
      if (err) return errorReply(reply, "payment", doc);
      return successReply(reply, "payment-pdf-file", doc);
    }
  );
};
...
exports.adminLogout = function(request, reply) {
  request.cookieAuth.clear();
...
exports.registerUser = function applyUser(request, reply) {
  ...
  const user = cloneWithProperties(data, [
    "name",
    "email",
    "phone",
    "password",
...
exports.addProfile = function(request, reply) {
  let user = cloneWithProperties(request.payload, [
    "email",
    "course-type",
    "basic-program",
    "cost",
  ...
  getDocument(request, (err, file) => {
    ...
    user.document = file;
    let totalCost =
      Number(user["cost"].slice(1).split(",").join("")) + (2000.00 + 3000.00);
    user.totalCost = formatMoney(totalCost);
...
function getDocument(request, cb) {
  ...
  if (payload.file) {
    let name = payload.email + "__" + payload.file.hapi.filename;
...
function setCookie(request, data, callback) {
  const sid = String(++request.server.app.uuid);
  let user = cloneWithProperties(data, ["_id", "email", "name", "role"]);

  // TODO: Implement persistent login for 'keep me logged in'
...
function checkAdmin(request) {
  ...
  if (
    request.auth.credentials.email !== adminEmail &&
...
function loginUser(request, reply) {
  ...
  function login() {
    ...
    checkUp(data.email, (err, doc) => {
      if (!err && doc) {
        let res = bcryptValidatePasswords(
          data.password,
          doc,
          (err, isValid) => {
...
function generatePDF(request, reply) {
  Axios.get(
    request.server.info.uri + "/pdf-file/" + request.auth.credentials.email
  )
    .then(function(response) {
      ...
      request.server.app.db.users.update(
        {
    ...
    .catch(function(error) {
      return errorReply(reply, "payment", null);
    });
}
