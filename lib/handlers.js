...
exports.registerUser = function applyUser(request, reply) {
    ...
    Request.post(
        `https://www.google.com/recaptcha/api/siteverify?secret=6LeIxAcTAAAAAGG-vFI1TnRWxMZNFuojJ4WifJWe&response=${user['g-recaptcha-response']}`, {},
                ...
                let res = bcryptValidatePasswords(data.password, doc, (err, isValid) => {
                    ...
                    if (isValid) {
                        setCookie(request, doc, err => {
                            ...
                            if (doc.email === 'hillson@admin.com') { // take him to admin page
                                return reply.redirect('/admin');
                            }
